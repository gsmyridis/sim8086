...
impl MovOp {
    ...
    pub fn try_parse_im_rm(bytes: &[u8]) -> DResult<Self, &[u8]> {
        let width = Width::parse_byte(bytes[0]);
        let mode = Mode::try_parse_byte(bytes[1])?;
        assert_eq!(Into::<u8>::into(Reg::parse_byte_mid(bytes[1])), 0b000u8);
        let rm = RM::parse_byte(bytes[1]);
        let rest = &bytes[2..];
        ...
        let (dest, remaining) = Operand::register_or_memory(width.as_bool(), &mode, rm.as_u8(), &rest)?;

        Ok((MovOp::new(source, dest), remaining))
    }

    /// Parses an Immediate to Register MOV instruction.
    pub fn try_parse_im_reg(bytes: &[u8]) -> DResult<Self, &[u8]> {
        let width = Width::parse_byte_mid(bytes[0]);
