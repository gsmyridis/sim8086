...
impl Registers {
    pub fn get(&self, reg: Register) -> Value {
        match reg {
            Register::AX => Value::Word(u16::from_le_bytes(self.ax)),
            Register::AL => Value::Byte(self.ax[0]),
            Register::AH => Value::Byte(self.ax[1]),
            Register::BX => Value::Word(u16::from_le_bytes(self.bx)),
            Register::BL => Value::Byte(self.bx[0]),
            Register::BH => Value::Byte(self.bx[1]),
            Register::CX => Value::Word(u16::from_le_bytes(self.cx)),
            Register::CL => Value::Byte(self.cx[0]),
            Register::CH => Value::Byte(self.cx[1]),
            Register::DX => Value::Word(u16::from_le_bytes(self.dx)),
            Register::DL => Value::Byte(self.dx[0]),
            Register::DH => Value::Byte(self.dx[1]),
            Register::SP => Value::Word(u16::from_le_bytes(self.sp)),
            Register::BP => Value::Word(u16::from_le_bytes(self.bp)),
            Register::SI => Value::Word(u16::from_le_bytes(self.si)),
            Register::DI => Value::Word(u16::from_le_bytes(self.di)),
        }
    }
    ...
    pub fn set(&mut self, reg: Register, val: Value) {
        match val {
            Value::Byte(v) => {
                match reg {
                    Register::AL => self.ax[0] = v,
                    Register::AH => self.ax[1] = v,
                    Register::BL => self.bx[0] = v,
                    Register::BH => self.bx[1] = v,
                    Register::CL => self.cx[0] = v,
                    Register::CH => self.cx[1] = v,
                    Register::DL => self.dx[0] = v,
                    Register::DH => self.dx[1] = v,
...
impl SegmentRegisters {
    pub fn get(&self, segreg: SegmentRegister) -> Value {
        ...
        Value::Word(u16::from_le_bytes(bytes))
    }
...
impl fmt::Display for SegmentRegisters {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(f, "-------------------\n")?;
        write!(f, "- ES: {:x}\n", u16::from_le_bytes(self.es))?;
        write!(f, "- CS: {:x}\n", u16::from_le_bytes(self.cs))?;
        write!(f, "- SS: {:x}\n", u16::from_le_bytes(self.ss))?;
        write!(f, "- DS: {:x}\n", u16::from_le_bytes(self.ds))?;
        Ok(())
    }
}
